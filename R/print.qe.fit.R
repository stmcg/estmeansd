...
print.qe.fit <- function(x, ...) {
  ...
  if (length(x$beta.par) != 1) {
    cat("\nEstimated parameters of beta distribution:\n")
    print(x$beta.par)
  } else {
    cat("\nBeta distribution not fit\n")
  }
  cat("\nValues of objective functions corresponding to estimated parameters:\n"
      )
  print(x$values)
}
